...
    concernAndFeature: "최대 지불 의향", // ⭐️ 응급 상황 불만/필요 기능
    priority1: "1순위 정보",
    priority2: "2순위 정보",
    priceRange: "불만/필요 기능" // ⭐️ 최대 지불 의향
  };
  
  // ⭐️ 이전/구식 필드 이름을 최신 필드 이름에 매핑
  // (구식 데이터가 '응답 없음'이 되지 않도록 매핑 유지)
  const legacyMap = {
...
  // 3. submissions 렌더링 (가장 중요한 수정 부분)
...
      card.className = "record fade-in"; // ⭐️ 애니메이션 클래스 유지
      card.style.setProperty('--delay', `${index * 0.05}s`);

      // ⭐️ displayKeys에 정의된 3개 항목만 순회하며 렌더링
      let html = displayKeys
          .map(k => {
              const label = keyMap[k];
              let value = sub[k];
              
              // ⭐️ 값이 없거나 빈 문자열인 경우, legacyMap에서 대체할 값이 있는지 확인
...
              
              // '응답 없음'인 경우는 빨간색으로, 나머지는 주황색으로 표시하도록 스타일 추가 (CSS에 .record-item 스타일 필요)
...
      // ⭐️ submissions 탭이 활성화될 때만 fetchSubmissions 호출
...
  // 6. 초기 서버 데이터 로드 (submissions 탭이 기본이 아니므로, 초기 로드 시점에서는 렌더링되지 않음)
  // submissions 탭을 클릭하거나 폼 제출 후 자동으로 전환될 때만 렌더링되도록 로직 변경
  // fetchSubmissions(); // ⭐️ 초기 로드에서 제거하여 중복 로딩 가능성을 줄임
